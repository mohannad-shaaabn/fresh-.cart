...
export default function Home() {
  ...
  function GetAllProducts() {
    return axios.get(
      `https://ecommerce.routemisr.com/api/v1/products?limit=25&page=${page}`,
    );
  }
  ...
  return (
    <>
      ...
      {isLoading ? (
        ...
        <div className="w-11/12 md:w-10/12 mx-auto my-6">
          <MainSlider />
          <CategorySlider />
          <div className="flex flex-wrap">
            {data?.data?.data?.map((product) => {
              let { _id, price, imageCover, category, title, ratingsAverage } =
                product;
              let { name } = category;
              return (
                <div
                  ...
                  <div className="item group overflow-hidden hover:border  hover:border-active p-4">
                    <Link to={`/ProductDetails/${_id}`}>
                      ...
                      <div className="flex justify-between">
                        <span>{price}EGP</span>
                        <span>
                          <i className="fa-solid fa-star text-yellow-300 me-1" />
                          {ratingsAverage}
                        </span>
                      </div>
                    </Link>
                    <button
                      onClick={() => addCart(_id)}
                      className="btn duration-500 translate-y-0 
md:translate-y-24 
